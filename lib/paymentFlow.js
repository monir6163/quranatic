...
/* Create a PENDING Payment and open a gateway charge for it. The submission
   (`pendingData`) is stashed on the Payment and only turned into a real
   Order/Appointment/HandAppointment once the payment is verified, so a canceled
   or abandoned charge never creates a business record.
   Returns { payment_url, paymentId }; throws on any failure (caller returns a
   JSON error to the client). */
async function startCharge({ req, targetType, pendingData, amount, fullName, email, phone }) {
...
/* Create (or, for legacy up-front records, mark paid) the target once payment is
   verified. Called only by the winning atomic settle, and guarded so it creates
   the record exactly once. */
async function applyPaid(record) {
  try {
    // Deferred flow (the default): the real record doesn't exist yet — build it
    // now from the stashed submission, then link it back to the Payment.
    if (!record.targetId) {
      const d = record.pendingData || {};
      let created = null;
      if (record.targetType === "Order") {
        created = await Order.create({ ...d, paid: true });
      } else if (record.targetType === "Appointment") {
        created = await Appointment.create({ ...d, paid: true });
      } else if (record.targetType === "HandAppointment") {
        created = await HandAppointment.create({ ...d, verified: true });
      }
      if (created) {
        await Payment.updateOne({ _id: record._id }, { $set: { targetId: created._id } });
      }
      return;
    }

    // Legacy flow: the record was created up-front — just mark it paid.
